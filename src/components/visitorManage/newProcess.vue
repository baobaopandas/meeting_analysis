<template>
  <div class="accessRecord-wrapper">
    ...
    <div class="MeetingInfo">
      ...
      <el-input
              placeholder="请输入您预估分享时长"
              v-model="time"
              class ="time"
      >
        <template slot="prepend">分享时长</template>
      </el-input>
    </div>
    ...
    <dialog-bar v-model="falsemeeting" type="confirm" :title=user content="分享不得为空，议程创建失败" v-on:cancel="clickCancel()" ></dialog-bar>
    <dialog-bar v-model="falsemeeting2" type="confirm" :title=user content="未连接到数据库，议程创建失败" v-on:cancel="clickCancel()" ></dialog-bar>
  </div>
</template>
